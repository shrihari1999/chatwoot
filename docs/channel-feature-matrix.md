...
| Rich text formatting (bold/italic/etc.) | ▶ out | LINE | RESOLVED: the LineRenderer artifact (which emitted literal ' *text* ') and the editor's LINE marks were both removed — LINE is now plain text end to end, content passes through the renderer service untouched. Real styling would require Flex Messages, which render as cards and degrade quoting to altText only; judged not worth it. |
...
- LINE rich_formatting[out] — RESOLVED: the LineRenderer emitted literal markdown rather than styling, so it was removed along with the editor's LINE marks; F=no is now accurate and the code matches it. P stays ⚠️ partial because LINE *can* style text, but only inside Flex Messages.
...
| Rich text formatting (bold/italic/etc.) | ▶ out | ⚠️ partial | Plain text has no bold/italic; only Flex Message text/span supports weight and decoration (span has no italic). | ❌ no | Deliberately unsupported: `Channel::Line` has no marks/nodes in the editor FORMATTING table, and is absent from `CHANNEL_RENDERERS` so content is sent verbatim. The old LineRenderer wrapped bold in ' * ' — displayed literally by LINE — and was removed. |
