...
const team = computed(() => props.chat.meta?.team || {});

// Fork addition: show the assigned team chip on the card.
const shouldShowTeam = computed(() => props.showTeamName && !!team.value.id);

const showMetaSection = computed(() => {
  return (
    props.showInboxName ||
    shouldShowTeam.value ||
  ...
  >
    ...
    >
      <Avatar
        ...
        :class="showInboxName || shouldShowTeam ? 'mt-8' : 'mt-4'"
        hide-offline-status
      >
    ...
    <div class="px-0 py-3 flex-1 min-w-0 border-line">
      ...
      >
        <div
          v-if="showInboxName || shouldShowTeam"
          class="flex items-center gap-2 flex-1 min-w-0"
        >
          <InboxName v-if="showInboxName" :inbox="inbox" class="min-w-0" />
          <TeamName v-if="shouldShowTeam" :team="team" class="min-w-0" />
        </div>
        <div
          class="flex items-baseline gap-2 flex-shrink-0"
          :class="{
            'flex-1 justify-between': !showInboxName && !shouldShowTeam,
          }"
        >
          <span
