class Sla::EvaluateAppliedSlaService
  ...
  # A threshold of 0 is treated the same as blank — an unconfigured SLA target
  # must never fire an instant miss (fork divergence).
  def check_frt
    return if sla_policy.first_response_time_threshold.to_f.zero?
  ...
  def check_nrt
    return if sla_policy.next_response_time_threshold.to_f.zero?
  ...
  def check_rt
    return if sla_policy.resolution_time_threshold.to_f.zero?
