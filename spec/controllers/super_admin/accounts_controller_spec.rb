...
RSpec.describe 'Super Admin accounts API', type: :request do
  ...
  describe 'PATCH /super_admin/accounts/{account_id} feature flags' do
    ...
    it 'enables a checked flag stored in a secondary bitmask column without raising' do
      # whatsapp_manual_transfer lives in feature_flags_ext_1 (a non-primary
      # column). Checking it via the admin form must route through that column's
      # bulk setter rather than raising ArgumentError "Invalid flag".
      patch "/super_admin/accounts/#{account.id}", params: {
        account: { name: account.name },
        enabled_features: { 'feature_whatsapp_manual_transfer' => 'true' }
      }

      expect(response).to have_http_status(:redirect)
      account.reload
      expect(account.feature_whatsapp_manual_transfer?).to be(true)
    end

    it 'keeps advanced_assignment enabled for a Business plan account via the enterprise sync' do
